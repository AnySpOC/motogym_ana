...
| FR-03 | Calibration操作で静止補正を開始し、成功または失敗理由を表示すること |
| FR-04 | Auto待機とAuto解除により自動発進待ちを制御できること |
| FR-05 | 手動開始と計測停止により計測を明示的に開始、終了できること |
...
| 状態 | 内容 | 主な遷移 |
| --- | --- | --- |
| `idle` | 初期待機 | Sensor ONで`sensor-on` |
| `calibrating` | Calibration操作後の4秒間静止補正 | 成功または失敗後`sensor-on` |
| `sensor-on` | センサー取得中、未計測 | Calibrationで`calibrating`、Auto待機で`armed`、手動開始で`running` |
| `armed` | 自動発進待ち | 前後Gしきい値超過で`running` |
| `running` | 計測中 | GPSを含む自動停止、計測停止、Sensor OFFで`stopped` |
| `stopped` | 計測完了、保存済み | Auto待機、手動開始またはSensor OFF |
| `sensor-off` | センサー停止 | Sensor ONで`sensor-on` |

Auto待機と手動開始はキャリブレーション成功後だけ有効になる。補正状態は`not-run`、`running`、`complete`、`failed`で保持し、失敗時は理由を画面に残す。
`sensor-on`では権限取得とGPS測位準備だけを行い、速度積分、グラフ追加、走行サンプル保存は行わない。`armed`ではSTART判定に必要な加速度だけを評価し、確定前の表示速度は0とする。
...
Sensor ONはセンサー取得だけを開始する。利用者がCalibrationを押した後、`Tcal = 4.0 s`の静止サンプル平均から次を求める。
...
エンジン振動は正負が高速に反転するため`a_lp`では減衰し、持続する車体加減速は残る。`a_c - a_lp`の二乗平均を時定数`0.35 s`で追跡し、振動量`vibrationG`として内部保持する。自動STARTは前後Gがしきい値を`140 ms`継続した場合にだけ成立させ、計測開始時刻は継続判定の先頭へ戻す。この確認区間の前後Gは仮積分して現在速度へ引き継ぎ、STARTイベントは時刻0、速度0として記録する。

静止補正は、START待機時と同じエンジンアイドリング状態で実行することを推奨する。回転数や取付状態が大きく変わった場合は、再度Calibrationを実行する。
...
```text
a_drive = 0.75 * measured_long_g + 0.25 * a_long_(k-1)
v_k = max(0, v_(k-1) + (a_drive * g0 - c_d * v_(k-1)) * dt)
...
`Q`は速度、加速度、ヨー、バンクのモデル誤差を表す。速度のプロセスノイズは`0.18 * dt`とし、GPS速度更新への追従性を確保する。`dt`は`0.005`から`0.12 s`へ制限し、Safari停止復帰時の過大積分を防ぐ。
...
低運動状態に加えて停止根拠が450 ms続いた場合だけ、速度をゼロへ拘束する。走行中の低運動状態だけでは等速走行と区別できないため、停止とは判定しない。

```text
|a_long| < 0.035 G
|a_lat| < 0.040 G
|yaw_rate| < 3.5 deg/s
かつ、GPSが2.5秒以内の観測で0.8 m/s未満
または、GPSなしで推定速度0.35 m/s未満かつ3.5秒以内に減速を検出
```
...
自動停止は低運動状態に加え、GPS停止または低速かつ直前の減速という停止根拠が950 ms継続し、START後1.5秒以上経過した場合に成立する。同種イベントは900 ms以内の再登録を抑制する。
...
- 低運動だけでは走行中の速度が0へ拘束されないこと
- 新鮮なGPS停止観測で速度が0へ戻り自動停止すること
- GPS観測で速度状態が補正されること
- CSVの全行が同じ列数になること
